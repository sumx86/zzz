...
(function($){
    $.extend(jQuery, {
        ...
        doAjax: function(opts, showLoading, parent) {
            ...
            if( showLoading && (parent != undefined && parent != null) ) {
                ui.loaderShow(parent.parent());
            }
            return $.ajax(params);
        }
    });
...
(function($) {
    $(document).ready(function() {
        $('.lang').click(function() {
            cookieUtil.create({
                    name: 'lang',
                    value: $(this).find('span').text().toLowerCase(),
                    path: '/',
                    days: 365
                }
            );
        ...
        if( cookieUtil.get('logo-tooltip') ) {
            $('.logo-tooltip').css('display', 'none');
        } else {
            $('.logo-tooltip').css('display', 'inline-block');
            $('.hide-tooltip').click(function(e) {
                cookieUtil.create({
                    name: 'logo-tooltip',
                    value: '1',
                    path: location.href,
...
(function($) {
    $.initCall('form-handling', {
        initialize: function() {
            ...
            $._on(document, '#sign-in-form, #registration-form, #reset-pass-form', {
        ...
        _initializeInputErrors: function() {
            $('.input-field').each(function(i, e) {
                ...
                if( $.inArray(form.attr('data-action'), ['login', 'register', 'reset-pass']) ) {
        ...
        _toggleSubmitLock: function() {
            $(document).find("[data-action='sign']").each(function(i, e) {
        ...
        _handleError: function(errObj) {
            $.each(errObj, function(i, e) {
                $("input[name='"+i+"']").css('border', '1px solid #c3382e').addClass('error');
                if( e != '' ) {
                    $('#'+i+'-error').css('display', 'block').text(e);   
                }
            });
        },
        _resetInputFields: function(form) {
            form.find('input').each(function(i, e) {
                var _field = $(e);
                _field.css('border', '1px solid #2a3232').removeClass('error'), $('#'+$(e).attr('name')+'-error').text('');
            });
        },
        _toggleForms: function() {
            var forms = ['#main-sign_in-container', '#main-sign_up-container'];
